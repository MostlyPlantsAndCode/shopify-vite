  ...
  return {
    ...
    configureServer({ config: config2, middlewares, httpServer }) {
      ...
      if (tunnelConfig.frontendPort !== -1) {
        ...
        config2.server.allowedHosts = [new URL(tunnelConfig.frontendUrl).hostname];
      }
        ...
        if (isAddressInfo(address)) {
          ...
          setTimeout(() => {
            void (async () => {
              ...
              config2.server.allowedHosts = [new URL(tunnelUrl).hostname];
              isTTY() && renderInfo({ body: `${viteDevServerUrl} is tunneled to ${tunnelUrl}` });
...
var viteTagEntryPath = (resolveAlias, entrypointsDir, snippetName) => {
  ...
  resolveAlias.forEach((alias) => {
    if (typeof alias.find === "string") {
      replacements.push([alias.find, normalizePath3(path4.relative(entrypointsDir, alias.replacement))]);
    }
  });
...
var scriptTag = (fileName, versionNumbers) => `<script src="{{ ${assetUrl(fileName, versionNumbers)} }}" type="module" crossorigin="anonymous"></script>`;
